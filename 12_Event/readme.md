...
同时`from`和`to`前面带着`indexed`关键字，每个`indexed`标记的变量可以理解为检索事件的索引“键”，在以太坊上单独作为一个 topic 进行存储和索引，程序可以轻松的筛选出特定转账地址和接收地址的转账事件。每个事件最多有3个带`indexed`的变量。事件的哈希以及这三个带`indexed`的变量在英文技术文档往往也被称为 `topic[0]` 到 `topic[2]`。每个 `indexed` 变量的大小为固定的256比特。
